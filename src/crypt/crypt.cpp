...
namespace crypt
{
    ...
    crypt::crypt()
    {
        ...
        if(ret!=1)
            LOG_ERROR<<"[crypt] BN_pseudo_rand err!";
        RAND_seed(rnd_seed, sizeof rnd_seed);
        ret=RSA_generate_key_ex(prsa,BITS,bne,NULL);
        if(ret!=1)
            LOG_ERROR<<"[crypt] RSA_generate_key_ex err!";
        ...
        {
            FLEN = 0;
            LOG_ERROR<<"[crypt] RSA_check_key err!";
        }
        //RSA_print_fp(stdout,prsa,11);

    /*
        if(save_key_rsa_public())
            printf("public key saved.\r\n");
        else
            printf("save public key error!\r\n");
        if(save_key_rsa_private())
            printf("private key saved.\r\n");
        else
            printf("save private key error!\r\n");
    */
        if(load_key_RSA()<=0)
        {
            LOG_ERROR<<"[crypt] load key error!";
        }
    ...
    int crypt::load_key_RSA(void)
    {
        ...
        {
            LOG_ERROR<<"[crypt] load private key error!";
        }
        if(load_key_rsa_public()<=0)
        {
            LOG_ERROR<<"[crypt] load public key error!";
            return 0;
        }
        return 1;
    }
    ...
    int crypt::rsa_encrypt(unsigned char *datain, unsigned char *dataout)
    {
        ...
        if (ret < 0)
        {
            LOG_ERROR<<"[crypt] RSA Encrypt failed!";
            return 0;
        }
    ...
    int crypt::rsa_decrypt(unsigned char *datain, unsigned char *dataout)
    {
        ...
        if (ret < 0)
        {
            LOG_ERROR<<"[crypt] rsa Decrypt failed!";
            return 0;
        }
        return ret;
    }
        ...
        if(count>0)
        {
            ...
            //{
                ...
                if(save_file((const char*)FILE_SN_NAME, (unsigned char*)strencode, count, true)>0)
                    LOG_DEBUG<<"[crypt] sn key generated.";
                else
                    return 0;
            //}
        }
    ...
    int crypt::rsa_generate_key_active(char *datain)
    {
        ...
        if(ret>0)
        {
            ...
            if(save_file((const char*)FILE_ACTIVE_NAME, (unsigned char*)strmd5, ret,true)>0)
                LOG_DEBUG<<"[crypt] active key generated.";
            else
                return 0;
        }
        else
